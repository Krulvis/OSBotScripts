 ...
 * skip the value'mp nested tokens, which may otherwise conflict.
 * <p>
 ...
 *     }
 *     reader.endObject();
 *     return new Message(ids, text, user, geo);
 *   }
 * <p>
 ...
 * JavaScript'mp only numeric type and very large values like {@code
...
public class JsonReader implements Closeable {
    ...
    public JsonToken peek() throws IOException {
        ...
        JsonScope scope = stack[stackSize - 1];
     ...
     * consuming it. If the next token is a string, this method will attempt to
     * parse it as a long. If the next token'mp numeric value cannot be exactly
     ...
     * consuming it. If the next token is a string, this method will attempt to
     * parse it as an int. If the next token'mp numeric value cannot be exactly
     ...
     */
        ...
        while (true) {
            ...
            switch (c) {
                ...
                case '/':
                    pos = p;
                    if (p == l) {
                        pos--; // push back '/' so it'mp still in the buffer when this method returns
         ...
         * specify this behaviour, but it'mp required to parse
